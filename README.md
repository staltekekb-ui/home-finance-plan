...
Приложение для учёта личных финансов с распознаванием скриншотов и PDF выгрузок из банковских приложений.
...
### Транзакции
- Загрузка скриншотов и PDF выгрузок из банковских приложений (одиночная и пакетная)
...
- **Обязательная привязка к счёту** — все транзакции привязываются к выбранному счёту
- **Умный выбор счёта** — приложение предлагает выбрать счёт после распознавания транзакций
- **Защита от ошибок** — невозможно добавить транзакции без создания счёта
- **Полная синхронизация** — изменения мгновенно отражаются во всех разделах
- **Обнаружение дубликатов** — автоматическая проверка при загрузке PDF/скриншотов
- **Компактное отображение дубликатов** — оранжевая граница + небольшой бейдж
- **Исключение дубликатов** — кнопки для удаления отдельных или всех дубликатов
- **Массовое удаление** — удаление по периоду, категории, счёту, типу транзакций
- **Выборочное удаление** — checkbox для каждой транзакции + кнопка "Удалить выбранные"
- Связывание с целями накоплений и счетами
...
- **Автоматическое распределение остатка** по целям после сохранения транзакций
- **Списание расходов с целей** при трате накопленных средств
- Ручное и автоматическое распределение средств
- Валидация достаточности средств при списании
...
- **Обязательная привязка транзакций** к счетам
- **Автоматический пересчёт баланса** при добавлении транзакций
- **Защита от добавления транзакций** без создания хотя бы одного счёта
...
- **Улучшенная читаемость** — увеличенный базовый шрифт (17px)
- **Гибкий ввод данных** — нет ограничений на вводимые суммы
- **Реактивное обновление** — все разделы синхронизируются автоматически
- **Мотивирующие сообщения** — позитивная обратная связь при создании счетов, целей, бюджетов
- **Полезные подсказки** — ненавязчивые hints о создании счетов и целей
- **Keyboard shortcuts** — Enter/Escape для подтверждения/отмены в модальных окнах
...
│   │   │   ├── EditParsedTransactionModal.tsx
│   │   │   ├── ConfirmModal.tsx
│   │   │   ├── ProgressBar.tsx    # Прогресс-бары (линейные и полукруглые)
│   │   │   ├── RecurringPaymentCard.tsx
│   │   │   ├── SettingsModal.tsx
│   │   │   ├── SavingsDistributionModal.tsx  # Распределение средств по целям
│   │   │   ├── ExpenseDeductionModal.tsx     # Списание расходов с целей
...
| POST | `/api/transactions/bulk-delete` | Массовое удаление (по ID или фильтрам) |
| POST | `/api/transactions/check-duplicates` | Проверка дубликатов транзакций |
| GET | `/api/transactions/categories` | Список категорий |

### Загрузка
| Метод | URL | Описание |
|-------|-----|----------|
| POST | `/api/upload` | Загрузить один скриншот или PDF |
| POST | `/api/upload/batch` | Загрузить несколько скриншотов или PDF |
...
| POST | `/api/savings/goals/{id}/subtract` | Списать из накоплений |
...
## 🎯 Умное управление целями накоплений

Приложение автоматизирует процесс работы с целями накоплений:

### После сохранения транзакций:

1. **Рассчитывается остаток**: Доходы - Расходы
2. **Предлагается распределение** (если остаток > 0):
   - Вручную укажите сумму для каждой цели
   - Или используйте автоматическое распределение поровну
3. **Предлагается списание** (если были расходы):
   - Выберите цели, с которых нужно списать средства
   - Валидация не позволит списать больше, чем накоплено

### Преимущества:

- 📊 Полная прозрачность движения средств
- 🎯 Автоматическое отслеживание прогресса целей
- 💰 Связь расходов с целями накоплений
- ⚡ Мгновенная синхронизация с Dashboard
